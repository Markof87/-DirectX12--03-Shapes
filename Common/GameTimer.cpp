...
void GameTimer::Start() {
	...
	if (mStopped) {
		...
		//Since we are starting the timer back up, the current previouys time is not valid,
		//as it occured while paused. So reset it to the current time.
		mPrevTime = startTime;
...
void GameTimer::Stop() {
	...
	if (!mStopped) {
		...
		//Otherwise, save the time we stopped at, and set the Boolean flag indicating 
...
float GameTimer::TotalTime()const {
	...
	//Moreover, if we previously already had a pause, the distance mStopTime - mBaseTime
	//includes paused time, which we do not want to count. To correct this, we can
