...
void ShapesApp::Update(const GameTimer& gt) {
	...
	//Has the GPU finished processing the commands of the current frame resource? If not, wait until the GPU has completed commands up to this fence point.
...
void ShapesApp::UpdateObjectCBs(const GameTimer& gt) {
	...
	for (auto& e : mAllRitems) {
		...
		//Only update the cbuffer data if the constants have changed. This needs to be tracked per frame resource.
...
void ShapesApp::BuildRootSignature() {
	...
	CD3DX12_ROOT_SIGNATURE_DESC rootSignDesc(2, slotRootParameter, 0, nullptr, D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);
...
void ShapesApp::BuildDescriptorHeaps() {
	...
	//Need a CBV descriptor for each object for each frame resource, +1 for the perPass CBV for each frame resource.
